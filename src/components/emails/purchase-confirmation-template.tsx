...
export const PurchaseConfirmationTemplate: React.FC<PurchaseConfirmationTemplateProps> = ({ firstName, orderId, productListHtml }) => (
  <div style={{ fontFamily: 'sans-serif', lineHeight: '1.6', color: '#333' }}>
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px', border: '1px solid #ddd', borderRadius: '5px' }}>
      ...
      <p>Your order <strong>#{orderId.substring(0, 8)}</strong> has been confirmed and is being processed. You can find your detailed invoice attached to this email.</p>
      <h3 style={{ borderBottom: '1px solid #eee', paddingBottom: '10px' }}>Your Products</h3>
      <ul dangerouslySetInnerHTML={{ __html: productListHtml }}></ul>
      <p>You can also view your order details anytime by visiting your account:</p>
      <div style={{ textAlign: 'center', margin: '30px 0' }}>
        <a href={`${BASE_URL}/account/orders/${orderId}`} style={{ backgroundColor: '#007bff', color: 'white', padding: '12px 25px', textDecoration: 'none', borderRadius: '5px', fontSize: '16px' }}>View Order</a>
      </div>
